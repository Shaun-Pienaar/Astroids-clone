...
function preload(){
  //Load Images
  starBackgroundImage = loadImage('Star_Background2.png');
  spaceshipImage = loadImage('Spaceship2.png');
  for(let i = 1; i <= 5; i++){
    astroidImages.push(loadImage(`Astroid${i}.png`));
  }

  //Load Sounds
  explosionSfx = loadSound('ExplosionEffect.mp3');
  shootSfx = loadSound('ShootEffect.mp3');
  astroidBreakSfx = loadSound('AstroidbreakEffect.mp3');
  menuTrack = loadSound('Menutrack.mp3');
  for(let i = 1; i <= 3; i++){
    backgrounTracks.push(loadSound(`Backgroundtrack${i}.mp3`));
  }
...
async function getHighscores(){
  let response = await fetch('http://localhost:5000/highscores');
...
async function saveHighscores(){
  ...
  let response = await fetch('http://localhost:5000/highscores', request);
  let data = await response.json();
  console.log(data);
}
